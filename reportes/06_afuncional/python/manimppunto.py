...
class ProductoPunto(Scene):
    def construct(self):
        titulo = Text("Producto punto", font_size=48).to_edge(UP,buff=0.2)
        ...
        label_u = MathTex(r"\vec{u} = (1, 1)", color=BLUE)
        label_u.next_to(u, UP, buff=0.1).shift(RIGHT * 0.5)

        label_v = MathTex(r"\vec{v} = (1, 0)", color=RED)
        label_v.next_to(v, UP, buff=0.1).shift(RIGHT * 1.5)
