...
* El capacitor se comporta como un circuito abierto para la corriente continua, se opone a
  cambios abrupto de voltaje.
...
La siguiente figura muestra un circuito RC y se modela por una ecuacion diferencial lineal ordinaria de primer orden (tiene un solo **polo**).  
...
\
 ...
 La resistencia controla *qué tan rápido* puede cambiar el voltaje en el capacitor. Cerrar el interruptor significa aplicar un *escalón* de magnitud fija en este caso:
...
como un sistema en lazo cerrado con retroalimentación unitaria. Observe que 
el diagrama en lazo cerrado es una *equivalencia matemática* del circuito físico.

**Nota:** La **trayectoria directa** (en inglés: *forward path*) en rojo, se refiere a la ruta que sigue la señal desde la entrada del sistema hasta la salida, pasando a través de los bloques del controlador y la planta, sin incluir el lazo de realimentación (en azul). En el caso del RC, la retroalimentación 
es inherente
...
* Con un solo polo re al negativo $s=-1/RC$
* O bien , ya que $\tau = RC$ , el polo es $s=-\frac{1}{\tau}$
* Entre mas cerca queda el polo del eje imaginario, mas lento es el tiempo de extablcimiento
* 
