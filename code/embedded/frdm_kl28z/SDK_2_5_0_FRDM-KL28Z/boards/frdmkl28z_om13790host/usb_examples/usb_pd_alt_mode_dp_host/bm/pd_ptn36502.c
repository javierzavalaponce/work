...
#define ptn36502_I2cWriteRegister(ptn36502Instance, registerAdd, data, dataLen)                                     \
    CMSIS_PortControlInterfaceWriteRegister(ptn36502Instance->cmsisHandle,                                          \
                                            ptn36502Instance->config->ptn36502SlaveAddress, registerAdd, 1, (data), \
                                            dataLen);
...
uint8_t PD_PTN36502Init(pd_ptn36502_instance_t *ptn36502Instance, pd_handle pdHandle, void *config)
{
    ...
    ptn36502Instance->config = (const pd_dp_redriver_config_t *)config;

    /* initialize I2C */
    CMSIS_PortControlInterfaceInit(&ptn36502Instance->cmsisHandle, ptn36502Instance->config->ptn36502I2CMaster, NULL);
    if (ptn36502Instance->cmsisHandle == NULL)
    {
        return 1;
    }
...
{
    CMSIS_PortControlInterfaceDeinit(ptn36502Instance->cmsisHandle);
    ptn36502Instance->cmsisHandle = NULL;
    return 0;
}
