...
pd_status_t PD_DpBoardChipsInit(void **interfaceHandle, pd_handle pdHandle, void *param)
{
    uint32_t index = 0;
    ...
    if (PD_CrossbarCBTLInit(&pdChipInstance->crossbarInstance, pdChipInstance->pdHandle,
                            ((pd_dp_peripheral_config_t *)param)->crossbarConfig) != 0)
    {
    ...
    if (PD_DpHpdDriverInit(&pdChipInstance->hpdDriverInstance, pdChipInstance->pdHandle,
                           ((pd_dp_peripheral_config_t *)param)->hpdDriverConfig) != 0)
    {
    ...
    if (PD_PTN36502Init(&pdChipInstance->ptn36502Instance, pdChipInstance->pdHandle,
                        ((pd_dp_peripheral_config_t *)param)->ptn36502Config) != 0)
    {
