...
static void PD_DpmPrintCableInfo(pd_app_t *pdAppInstance)
{
    ...
    else
    {
        ...
        else if (cablePlugInfo.cableType == kCableType_ActiveCable)
        {
            PRINTF("active");
...
pd_status_t PD_DpmSrcRDOResultCallback(void *callbackParam, uint8_t success, uint8_t failResultType)
{
    ...
    if (success)
    {
        ...
        if (pdAppInstance->powerRoleChangedOrAttached == 0)
        {
            pdAppInstance->powerRoleChangedOrAttached = 1;
    ...
    switch (type)
    {
        ...
        case kInfoType_ManufacturerInfo:
        {
            ...
            if ((getManufacInfo->manufacturerInfoRef != 0) || (getManufacInfo->manufacturerInfoTarget > 1))
            {
                 ...
                 * return zero bytes */
                /* there is no battery */
                dataParam->dataLength = PD_MANUFACTURER_INFO_DATA_SIZE;
            }
            else
            {
                dataParam->dataLength = PD_MANUFACTURER_INFO_DATA_SIZE + 3;
            }
