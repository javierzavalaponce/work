...
{
    uint32_t eventSet;
    PD_TimerStart(pdInstance, timrMsgSendWaitResultTimer, T_WAIT_SEND_RESULT);
    do
    {
        eventSet = 0u;
        ...
        {
            PD_TimerClear(pdInstance, timrMsgSendWaitResultTimer);
        ...
        if ((PD_ConnectCheck(pdInstance) == (uint8_t)kConnectState_Disconnected) ||
            (PD_TimerCheckInvalidOrTimeOut(pdInstance, timrMsgSendWaitResultTimer)))
        {
            PD_TimerClear(pdInstance, timrMsgSendWaitResultTimer);
            PD_PhyControl(pdInstance, PD_PHY_CANCEL_MSG_TX, NULL);
            pdInstance->sendingState = 0;
                                      ...
                                      uint8_t *dataBuffer)
{
    ...
    if (extMsgHeader->bitFields.chunked)
    {
        for (; index < (objCount * 4 - 2); ++index)
        {
            uint8Buffer[index] = 0x00u;
        }
    }
