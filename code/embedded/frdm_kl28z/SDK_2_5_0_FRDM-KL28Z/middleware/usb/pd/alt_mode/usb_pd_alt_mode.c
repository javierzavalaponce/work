                ...
                case kVDM_DiscoverSVIDs: /* UFP */
                {
                    ...
                    for (index = 0; index < altModeInstance->altModeConfig->altModeHostConfig.moduleCount; ++index)
                    {
                        ...
                        if (!(index & 0x01u))
                        {
                            ...
                            svidsObj |=
                                ((uint32_t)altModeInstance->altModeConfig->altModeHostConfig.modules[index].SVID << 16);
                        }
                        else
                        {
                            svidsObj &= 0xFFFF0000u;
                            svidsObj |= altModeInstance->altModeConfig->altModeHostConfig.modules[index].SVID;
                        }
