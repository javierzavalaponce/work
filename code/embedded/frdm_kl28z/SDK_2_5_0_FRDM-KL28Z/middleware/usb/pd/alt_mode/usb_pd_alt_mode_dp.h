...
{
    /*! USB r2.0 signaling may be required on A6 每 A7 or B6 每 B7 while in DisplayPort Configuration */
    kUSB2_Required = 0,
    /*! USB r2.0 signaling is not required on A6 每 A7 or B6 每 B7 while in DisplayPort Configuration */
    kUSB2_NotRequired = 1,
...
#endif
    /*! enable HPD detect */
    kDPPeripheal_ControlHPDDetectEnable,
    /*! disable HPD detect */
    kDPPeripheal_ControlHPDDetectDisable,
...
typedef struct _pd_dp_board_peripheral_interface
{
    ...
    pd_status_t (*dpPeripheralInit)(void **interfaceHandle, pd_handle pdHandle, void *param);
...
#if (defined PD_CONFIG_ALT_MODE_SLAVE_SUPPORT) && (PD_CONFIG_ALT_MODE_SLAVE_SUPPORT)
/**
* @brief DisplayPort mode slave config
*/
typedef struct _pd_alt_mode_dp_slave_config
{
    ...
    /*! modes count */
    uint32_t modesCount;
    /*! displayport connection config */
    uint32_t displayPortConnection : 2;
    /*! adapter power low config */
    uint32_t adapterPowerLow : 1;
    /*! adapter enabled state config */
    uint32_t adapterEnabled : 1;
    /*! multi function preferred config */
    uint32_t multiFunctionPreferred : 1;
    uint32_t reserved : 27;
} pd_alt_mode_dp_slave_config_t;
#endif

/**
* @brief DisplayPort mode host config
*/
typedef struct _pd_alt_mode_displayport_host_config
{
    /*! customer can define this structure by self
     * For example: hpd/crossbar driver can differ for different customers' solution.
     */
    ...
    /*! pd_dp_peripheral_interface_t */
    const pd_dp_peripheral_interface_t *peripheralInterface;
    /*! supported pin assignments */
    uint32_t supportPinAssigns : 8;
    /*! multi function prefered */
    uint32_t multiFunctionPrefered : 1;
    uint32_t reserved : 23;
} pd_alt_mode_dp_host_config_t;
