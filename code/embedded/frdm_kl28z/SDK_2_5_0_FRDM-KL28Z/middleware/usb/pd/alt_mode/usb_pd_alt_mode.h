...
typedef struct _pd_alt_mode_module
{
    /*! module's SVID */
    uint32_t SVID;
    /*! module specific configure parameter, for example: pd_alt_mode_dp_host_config_t for displayport host */
    const void *config;
} pd_alt_mode_module_t;

/**
* @brief PD alternating mode config for slave (UFP)
*/
typedef struct _pd_alt_mode_slave_config
{
...
} pd_alt_mode_slave_config_t;

/**
* @brief PD alternating mode config for host (DFP)
*/
typedef struct _pd_alt_mode_host_config
{
    /*! module's list, the first module has high priority */
    const pd_alt_mode_module_t *modules;
    /*! module count for modules */
    uint32_t moduleCount;
} pd_alt_mode_host_config_t;
...
typedef struct _pd_alt_mode_config
{
     ...
     */
    uint8_t altModeRole;
    /*! pd_alt_mode_host_config_t or pd_alt_mode_slave_config_t */
    union
    {
        /*! alternating mode config for host (DFP) */
        pd_alt_mode_host_config_t altModeHostConfig;
        /*! alternating mode config for slave (UFP) */
        pd_alt_mode_slave_config_t altModeSlaveConfig;
    };
 ...
 * 0 - wait exiting alt mode, then do dr swap;
 * 1 - accept exiting alt mode;
 * 2 - reject exiting alt mode;
 */
