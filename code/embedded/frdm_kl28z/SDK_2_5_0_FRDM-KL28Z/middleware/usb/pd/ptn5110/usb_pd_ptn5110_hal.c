...
pd_status_t PDPTN5110_HalInit(pd_phy_handle pdPhyHandle)
{
    pd_phy_ptn5110_instance_t *ptn5110Instance = (pd_phy_ptn5110_instance_t *)pdPhyHandle;

    if (Reg_BusReadBlock(ptn5110Instance, product_id, 2, (uint8_t *)&ptn5110Instance->tcpcRegCache.GLOBAL.product_id) !=
            ARM_DRIVER_OK ||
        ptn5110Instance->tcpcRegCache.GLOBAL.product_id != PRODUCT_ID_PTN5110)
    {
    ...
    if (Reg_CacheRead(ptn5110Instance, GLOBAL, pd_interface_rev) < PD_INTERFACE_REV2VER1)
    {
         ...
         * Update to match TCPCv2 */
        Reg_CacheAndBusModifyWordField(ptn5110Instance, VBUS, vbus_sink_disconnect_threshold,
...
void PDPTN5110_EnableFRSwap(pd_phy_ptn5110_instance_t *ptn5110Instance)
{
    ...
    {
        if ((ptn5110Instance->phyPowerRole == kPD_PowerRoleSink) &&
        ...
        if (RegValue & TCPC_ALERT_FAULT_MASK)
        {
                               ...
                               TCPC_FAULT_STATUS_INTERNAL_OR_EXTERNAL_VBUS_OVER_VOLTAGE_PROTECTION_FAULT_MASK))
            {
                ...
                if (!(faultStatus & TCPC_FAULT_STATUS_MASK_ALL_REGISTERS_RESET_TO_DEFAULT_INTERRUPT_STATUS_MASK_MASK))
                {
                    ...
                    PD_Notify(ptn5110Instance->pdHandle, PD_PHY_EVENT_TYPEC_0VP_OCP_FAULT, &faultStatus);
