...
pd_status_t PDPTN5110_Init(pd_handle upperLayerHandle, pd_phy_handle *pdPhyHandle, pd_phy_config_t *phyConfig)
{
    ...
    *pdPhyHandle = ptn5110Instance;
    ...
    if (PDPTN5110_HalInit(ptn5110Instance) != kStatus_PD_Success)
    {
...
pd_status_t PDPTN5110_Control(pd_phy_handle pdPhyHandle, uint32_t control, void *param)
{
    ...
    switch (control)
    {
        case PD_PHY_SET_TYPEC_VBUS_ILIM:
        {
            /* TODO */
            break;
        }
        ...
        case PD_PHY_GET_LOOK4_CONNECTION_STATE:
            ...
            if (Reg_CacheRead(ptn5110Instance, CC_STATUS, cc_status) & TCPC_CC_STATUS_LOOKING4_CONNECTION_MASK)
            {
                *((uint8_t *)param) = kLook4ConnState_Looking;
            }
            else
            {
                if (Reg_CacheRead(ptn5110Instance, CC_STATUS, cc_status) & TCPC_CC_STATUS_CONNECT_RESULT_MASK)
                {
                    *((uint8_t *)param) = kLook4ConnState_AssertRd;
                }
                else
                {
                    *((uint8_t *)param) = kLook4ConnState_AssertRp;
                }
            }
            break;
        ...
        case PD_PHY_INFORM_VBUS_VOLTAGE_RANGE:
            if (Reg_CacheRead(ptn5110Instance, GLOBAL, pd_interface_rev) >= PD_INTERFACE_REV2VER1)
            {
                uint32_t vbusVoltage = *((uint32_t *)param);
                paramVal = (vbusVoltage & 0x0000FFFFu);
