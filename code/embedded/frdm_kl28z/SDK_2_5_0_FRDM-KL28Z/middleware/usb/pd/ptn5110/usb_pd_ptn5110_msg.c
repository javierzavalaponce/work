...
uint8_t PDPTN5110_MsgHalSendControl(pd_phy_ptn5110_instance_t *ptn5110Instance, uint8_t sop, uint8_t type)
{
...

    {
        if (!PDPTN5110_MsgHalSendCommonPrefix(ptn5110Instance, sop))
        {
            ...
            if (0 != Reg_BusWriteBlock(ptn5110Instance,
                                       (uint8_t *)&ptn5110Instance->msgTxBuf[TCPC_TRANSMIT_BYTE_COUNT_POS],
                                       TCPC_TRANSMIT_BYTE_COUNT_LEN + TCPC_TX_BUF_HEADER_LEN, transmit_byte_count))
            {
                return retValue;
            }
...
uint8_t PDPTN5110_MsgHalSendData(pd_phy_ptn5110_instance_t *ptn5110Instance, uint8_t sop, uint8_t type, uint8_t count)
{
...

    {
        ...
        if (0 != Reg_BusWriteBlock(ptn5110Instance, (uint8_t *)&ptn5110Instance->msgTxBuf[TCPC_TRANSMIT_BYTE_COUNT_POS],
