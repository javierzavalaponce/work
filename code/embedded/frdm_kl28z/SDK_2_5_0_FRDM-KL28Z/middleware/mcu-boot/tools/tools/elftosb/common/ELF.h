...
typedef /*off_t*/ uint64_t Elf64_Off;
...
{
	unsigned char e_ident[EI_NIDENT]; //!< Magic number identifying the file format.
	Elf64_Half e_type;                //!< Identifies the object file format.
	Elf64_Half e_machine;             //!< Specified the architecture for the object file.
	Elf64_Word e_version;             //!< Object file version.
	Elf64_Addr e_entry;               //!< Virtual address of the entry point, or 0.
	Elf64_Off  e_phoff;                //!< Program header table offset in bytes, or 0 if no program header table.
	Elf64_Off  e_shoff;                //!< Section header table offset in bytes, or 0 if no section header table.
	Elf64_Word e_flags;               //!< Processor-specific flags associated with the file.
	Elf64_Half e_ehsize;              //!< The ELF header's size in bytes.
	Elf64_Half e_phentsize;           //!< Size in bytes of one entry in the program header table.
	Elf64_Half e_phnum;               //!< Number of entries in the program header table.
	Elf64_Half e_shentsize;           //!< Size in bytes of an entry in the section header table.
...
struct Elf64_Shdr
{
	Elf64_Word sh_name;      //!< The section's name. Index into the section header string table section.
	Elf64_Word sh_type;      //!< Section type, describing the contents and semantics.
	Elf64_XWord sh_flags;     //!< Section flags describing various attributes.
	Elf64_Addr sh_addr;      //!< The address at which the section will appear in the memory image, or 0.
	Elf64_Off  sh_offset;     //!< Offset from beginning of the file to the first byte in the section.
	Elf64_XWord sh_size;      //!< The section's size in bytes.
	Elf64_Word sh_link;      //!< Section header table link index. Interpretation depends on section type.
	Elf64_Word sh_info;      //!< Extra information about the section. Depends on section type.
	Elf64_XWord sh_addralign; //!< Address alignment constraint. Values are 0 and positive powers of 2.
	Elf64_XWord sh_entsize; //!< Size in bytes of section entries, or 0 if the section does not have fixed-size entries.
};
/*!
...
struct Elf64_Phdr
{
	Elf64_Word p_type;   //!< What type of segment this header describes.
	Elf64_Word p_flags;  //!< Flags relevant to the segment.
	Elf64_Off p_offset;  //!< Offset in bytes from start of file to the first byte of the segment.
	Elf64_Addr p_vaddr;  //!< Virtual address at which the segment will reside in memory.
	Elf64_Addr p_paddr;  //!< Physical address, for systems where this is relevant.
	Elf64_XWord p_filesz; //!< Number of bytes of file data the segment consumes. May be zero.
	Elf64_XWord p_memsz;  //!< Size in bytes of the segment in memory. May be zero.
	Elf64_XWord p_align; //!< Alignment constraint for segment addresses. Possible values are 0 and positive powers of 2.
};
...
struct Elf64_Sym
{
	Elf64_Word st_name;     //!< Index into file's string table.
	unsigned char st_info;  //!< Specified the symbol's type and binding attributes.
	unsigned char st_other; //!< Currently 0 (reserved).
	Elf64_Half st_shndx;    //!< Section header table index for this symbol.
	Elf64_Addr st_value;    //!< Value associated with the symbol. Depends on context.
	Elf64_XWord st_size;     //!< Size associated with symbol. 0 if the symbol has no size or an unknown size.
};
